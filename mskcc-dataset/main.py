...
# Pre-Processing
mutation[['temp', 'HGVSp_Short']] = mutation['HGVSp_Short'].str.split(
    ".", expand=True)
...
combined = pd.merge(patients, combined_mutations, on=[
                    'PATIENT_ID']).groupby(by=["PATIENT_ID"]).agg({
                        ...
                        'CANCER_TYPE': lambda x: list(x),
                        'CANCER_TYPE_DETAILED': lambda x: list(x),
                        'METASTATIC_SITE': lambda x: list(x),
                        'MUTATION': 'sum'
                    })
