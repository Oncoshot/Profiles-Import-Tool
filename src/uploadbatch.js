...
function uploadProfiles(profiles, command, batchSize) {
    authenticate().then(async (token, err) => {
        ...
        // Push each entry
        for (let i = 0; i < profiles.length; i+= batchSize, batchNumber++) {
            await new Promise(async (resolve, reject) => {
                // Set id
                var batch = profiles.splice(i, batchSize);
                const data = JSON.stringify(batch)

                const options = {
                    hostname: API_HOSTNAME,
                    path: `/api/v1/organizations/${organizationId}/profiles/${command}/batch`,
                    method: 'PUT',
                    headers: {
                ...
                const req = https.request(options, res => {
                    log += `#${batchNumber} batch: statusCode: ${res.statusCode}\n`
                    console.log(`#${batchNumber} batch: statusCode: ${res.statusCode}`)
...
let batchSize = Number.parseInt(process.argv[4]) || 1;

try {
    profiles = retrieveFile(inputFileName)
    console.log(`Profiles to upload : ${profiles.length}`)
    
    uploadProfiles(profiles, command, batchSize)
} catch (err) {
    if (err.code == "ENOENT") {
